...
TOOL_INPUT_PROPERTIES: dict[str, dict[str, Any]] = {
    ...
    "wait_for_instructions": {
        "loop_id": {
            "type": "string",
            "description": "Stable wait-loop UUID; omitted values are generated by Fulcrum.",
        },
    },
    "claim_action": {
        "attempt_id": {
            "type": "string",
            "description": "Stable invocation-attempt UUID; omitted values are generated by Fulcrum.",
        },
        "native_tool_use_id": {"type": "string"},
    },
    "report_action_result": {
        "attempt_id": {"type": "string"},
        "outcome": {
            "type": "string",
            "enum": ["succeeded", "rejected", "uncertain"],
        },
        "evidence": {},
        "native_result": {},
    },
...
def _schema(name: str) -> dict[str, Any]:
    ...
    properties: dict[str, Any] = {
        "request_id": {
            ...
            "format": "uuid",
            "description": (
                "Omit for a new operation. Reuse only to resume the exact same "
                "timed-out call."
            ),
        },
        "task_id": {"type": "string"},
...
def tool_descriptions() -> list[dict[str, Any]]:
    descriptions = {
        "register_standing": (
            ...
            "task_id and CODEX_SESSION_ID as session_id. Omit request_id for a new "
            "registration; Fulcrum generates one."
        ),
        "wait_for_instructions": (
...
class FreshCli:
    ...
    def invocation(
        ...
        if name == "claim_action" and not payload.get("attempt_id"):
            payload["attempt_id"] = str(uuid.uuid4())
        if name == "wait_for_instructions":
            payload.setdefault("loop_id", str(uuid.uuid4()))
            payload.setdefault("turn_id", str(uuid.uuid4()))
